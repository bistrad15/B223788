...
#Define list of anticoagulant drugs
...
#Creating a regular expression that acts as a search filter for the drugs
...
read_prescriptions_year <- function(year, root = "data") {
  ...
  files %>%
    map_dfr(~ read_csv(.x, show_col_types = FALSE, progress = FALSE)) %>% #binding all files together in a single dataframe
    clean_names() %>%
...
anti_coagulant_data <- anti_coagulant_data %>%
  mutate(
    paid_quantity = suppressWarnings(as.numeric(paid_quantity)), #convert paid_quantity variable in a numeric form 
    month_date = case_when(
      str_detect(paid_date_month, "^\\d{6}$") ~ ymd(paste0(paid_date_month, "01")), # allows formatting of the "Date" column to be consistent
...
anti_coagulant_data <- anti_coagulant_data %>%
  mutate(
    DrugGroup = case_when(
      ...
      TRUE ~ "Other" #the rest of the drugs in the dataset, can be used to visualize how the trends in anti-coagulants specifically changed, compared to other medicines over the years. Could be removed??
    )
  )
...
monthly_yearly_data_summary <- monthly_yearly_data_summary %>%
  mutate(Month = factor(Month, levels = month.abb, ordered = TRUE)) #sets the order of the months
